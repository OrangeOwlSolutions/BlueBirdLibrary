...
#ifndef __CUDAMULOP_CUH__
#define __CUDAMULOP_CUH__
...
namespace BB
{
	...
	class Mul 
	{
		public:
			...
			// --- Real-Complex
			__host__ __device__ __forceinline__ static Promotion<int,int2_>::strongest			eval(const int a, const int2_ b)			{ Promotion<int,int2_>::strongest				c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float,int2_>::strongest		eval(const float a, const int2_ b)			{ Promotion<float,int2_>::strongest				c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double,int2_>::strongest		eval(const double a, const int2_ b)			{ Promotion<double,int2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<int,float2_>::strongest		eval(const int a, const float2_ b)			{ Promotion<int,float2_>::strongest				c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float,float2_>::strongest		eval(const float a, const float2_ b)		{ Promotion<float,float2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double,float2_>::strongest		eval(const double a, const float2_ b)		{ Promotion<double,float2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<int,double2_>::strongest		eval(const int a, const double2_ b)			{ Promotion<int,double2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float,double2_>::strongest		eval(const float a, const double2_ b)		{ Promotion<float,double2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double,double2_>::strongest	eval(const double a, const double2_ b)		{ Promotion<double,double2_>::strongest			c; c.c.x = a*b.c.x; c.c.y = a*b.c.y; return c; }
			// --- Complex-Complex
			__host__ __device__ __forceinline__ static Promotion<int2_,int2_>::strongest		eval(const int2_ a, const int2_ b)			{ Promotion<int2_,int2_>::strongest				c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<int2_,float2_>::strongest		eval(const int2_ a, const float2_ b)		{ Promotion<int2_,float2_>::strongest			c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float2_,int2_>::strongest		eval(const float2_ a, const int2_ b)		{ Promotion<float2_,int2_>::strongest			c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<int2_,double2_>::strongest		eval(const int2_ a, const double2_ b)		{ Promotion<int2_,double2_>::strongest			c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double2_,int2_>::strongest		eval(const double2_ a, const int2_ b)		{ Promotion<double2_,int2_>::strongest			c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float2_,float2_>::strongest	eval(const float2_ a, const float2_ b)		{ Promotion<float2_,float2_>::strongest			c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<float2_,double2_>::strongest	eval(const float2_ a, const double2_ b)		{ Promotion<float2_,double2_>::strongest		c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double2_,float2_>::strongest	eval(const double2_ a, const float2_ b)		{ Promotion<double2_,float2_>::strongest		c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
			__host__ __device__ __forceinline__ static Promotion<double2_,double2_>::strongest	eval(const double2_ a, const double2_ b)	{ Promotion<double2_,double2_>::strongest		c; c.c.x = a.c.x*b.c.x-a.c.y*b.c.y; c.c.y = a.c.y*b.c.x+a.c.x*b.c.y; return c; }
	...
	//Scalar_Scalar(T1,T2,Mul,operator*) 	
	Matrix_Scalar_Matrix(T1,T2,Mul,operator*)
	Matrix_Scalar_CudaMatrix(T1,T2,Mul,operator*)
	Scalar_Matrix_Matrix(T1,T2,Mul,operator*)
	Scalar_Matrix_CudaMatrix(T1,T2,Mul,operator*)
	Expression_Scalar(Q,T1,T2,Mul,operator*)
	Scalar_Expression(Q,T1,T2,Mul,operator*)
	Matrix_Matrix_Matrix(T1,T2,Mul,operator*)
	Matrix_Matrix_CudaMatrix(T1,T2,Mul,operator*)
	Expression_Expression(Q1,T1,Q2,T2,Mul,operator*)
	Expression_Matrix_Matrix(Q,T1,T2,Mul,operator*)
	Expression_Matrix_CudaMatrix(Q,T1,T2,Mul,operator*)
	Matrix_Expression_Matrix(Q,T1,T2,Mul,operator*)
	Matrix_Expression_CudaMatrix(Q,T1,T2,Mul,operator*)
